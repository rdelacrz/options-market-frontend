...
import {
  getAdditionalFundDataFromAMMData,
  getFundDataFromMarketData,
...
export const useFundList = () => {
  const markets = useSelector<State, Market[]>(
    (state) => state.fundInfo.markets
  );
  const ammDataMap = useSelector<State, { [id: string]: AmmData }>(
    (state) => state.fundInfo.ammDataMap
  );

  const baseFundList = useMemo(
    () =>
      markets
        .map((market) => getFundDataFromMarketData(market))
  ...
  const throttleUpdate = useRef(
    throttle(
      (baseFundList: OptionsEntry[], ammDataMap: { [id: string]: AmmData }) => {
        const updatedFundList = baseFundList.map((optionsEntry) =>
          getAdditionalFundDataFromAMMData(optionsEntry, ammDataMap)
        );
